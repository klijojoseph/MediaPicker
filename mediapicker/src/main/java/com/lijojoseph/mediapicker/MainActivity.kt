...
import com.lijojoseph.trimmer.utils.FileUtils
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val startForResult =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
                when(result.resultCode){
                    RESULT_OK -> {
                        val fileName = result.data?.getStringExtra(MediaPicker.TAG_FILE_NAME)
                        Toast.makeText(this,"Result OK $fileName",Toast.LENGTH_SHORT).show()
                    }
                    RESULT_CANCELED -> {
        ...
        trimmerButton.setOnClickListener {
            MediaPicker.Builder()
                .setChooseType(MediaPicker.CHOOSER_GALLERY)
                .setMediaType(MediaPicker.TYPE_VIDEO)
